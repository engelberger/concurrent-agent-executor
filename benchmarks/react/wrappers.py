...
class HotPotQAWrapper(gym.Wrapper):
    def __init__(self, env, base_path, split):
        super().__init__(env)
        data_file = os.path.join(base_path, DATA_DIR, HOTPOTQA_SPLIT_FILE[split])
...
class FeverWrapper(gym.Wrapper):
    def __init__(self, env, base_path, split):
        ...
        data_path = os.path.join(base_path, DATA_DIR, FEVER_SPLIT_FILE[split])
...
class LoggingWrapper(gym.Wrapper):
    ...
    def reset(self, seed=None, return_info=False, options=None, idx=None):
        output = self.env.reset(
            seed=seed, return_info=return_info, options=options, idx=idx
        )
